...
In this section, we'll walk through the steps to configure OpenTofu to manage a NixOS virtual machine on a Proxmox server, from setting up permissions to writing and applying your infrastructure definitions.
...
3.  Next, create a file named `imports.tf` to import the existing NixOS VM into OpenTofu:
    ...
    import {
     ...
     id = "pve1/qemu/105" // Replace with your Proxmox node and VM ID
    }
...
7.  Run the following command to generate the OpenTofu configuration:
    ...
    ```bash frame="none"
      ...
      proxmox_vm_qemu.nixos: Preparing import... [id=pve1/qemu/105]
      proxmox_vm_qemu.nixos: Refreshing state... [id=pve1/qemu/105]
...
9.  After fixing the configuration errors, run the following commands to plan again, format the configuration, and apply the changes:
    ...
    ```bash frame="none"
     ...
     proxmox_vm_qemu.nixos: Modifying... [id=pve1/qemu/105]
     proxmox_vm_qemu.nixos: Modifications complete after 2s [id=pve1/qemu/105]
...
You have now successfully set up OpenTofu to manage a NixOS VM on Proxmox. You can now use OpenTofu to automate the management of your Proxmox resources, making it easier to deploy and maintain your NixOS VMs.
